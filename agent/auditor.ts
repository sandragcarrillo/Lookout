...
${txHash
  ? `**Onchain record:** [${txHash}](${cfg.explorer}/tx/${txHash})`
  : writeError
    ? `_Score not written onchain: ${writeError}_`
...
export async function audit(targetAddress: string, chain: Chain, opts?: { privateKey?: string }): Promise<AuditResult> {
  ...
  const runtimeKey = (opts?.privateKey ?? process.env.AUDITOR_PRIVATE_KEY ?? AUDITOR_PRIVATE_KEY) as `0x${string}`;
  if (runtimeKey && runtimeKey !== '0x') {
    log(`SUBMIT: writing score ${scoringResult.score} onchain...`);
    const writeStart = Date.now();
    try {
      txHash = await writeScoreOnchain(target, scoringResult.score, scoringResult.breakdown, reportCID, chain, runtimeKey);
      log(`Score written ✅ tx: ${txHash}`);
      log(`View: ${cfg.explorer}/tx/${txHash}`);
    } catch (err) {
      const msg = (err as Error).message ?? String(err);
      writeError = msg.includes('AuditTooFrequent')
        ? 'rate-limited — scores can only be updated once per hour per agent to prevent score manipulation'
        : msg.includes('NotAuditor')
          ? 'auditor wallet not authorised on this contract'
          : msg.slice(0, 120);
      warn(`Failed to write score onchain: ${writeError}`);
    }
    toolCalls.push({
    ...
    decisions.push({
      step: 5,
      decision: txHash ? `Score written onchain: ${txHash}` : `Onchain write failed: ${writeError}`,
    ...
    writeError = 'AUDITOR_PRIVATE_KEY env var not set';
    decisions.push({
      step: 5,
      decision: 'Skipped onchain write — no auditor private key',
      reasoning: writeError,
  ...
  const reportContent = generateReport(target, chain, scoringResult, ensName, selfVerified, txHash, runId, writeError);
